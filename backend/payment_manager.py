...
class Payment(Resource):
    def post(self):
        ...
        user_response = (
            db.table("User").select("password_hash").eq("user_id", args["user_id"]).execute()
        )
        if not user_response.data:
            return {
                "error": "User not found"
            }, 404
        stored_password = user_response.data[0]["password_hash"]
        if not check_password_hash(stored_password, args["password"]):
            return {
